...
The optional `OutcomeNotice` and `StudioStatusBar` components provide Nubbin's default publish
feedback. Import their stylesheet once in the host application:

```tsx
import "@nubbin/studio-ui/styles.css";
import { OutcomeNotice, StudioStatusBar } from "@nubbin/studio-ui";
```

Consumers that supply their own presentation do not need the stylesheet or either component.
